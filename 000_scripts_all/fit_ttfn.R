...
model <- '
  ...
  TD_turnover ~ TP + Urban + Agriculture + Forest +
                AT + AP + spatial_distance

  FD_nestedness ~ TD_turnover + TDxTPmin + TP +
                  Urban + Agriculture + Forest +
                  AT + AP + spatial_distance
...
mods_by_ds <- data_by_rep[[1]] |>   # any rep is fine for dataset-level constants
  ...
  data_1 <- data_i |>
    ...
    mutate(
      ...
      TD_turnover_c = TD_turnover - mean(TD_turnover, na.rm = TRUE),
      TPmin_c   = TPmin - mean(TPmin, na.rm = TRUE),
      
      # interaction term
      TDxTPmin = TD_turnover_c * TPmin_c
    ) |>
    ungroup() |> 
    as.data.frame()
  ...
  tp_row <- idx_row("TP")
  td_row <- idx_row("TD_turnover")
  fd_row <- idx_row("FD_nestedness")
  
  tp_col   <- idx_col("TP")
  td_col   <- idx_col("TD_turnover")
...
out_file <- glue::glue(
  "STtoFN/metaSEM_rep{sprintf('%03d', i)}.rds"
)
